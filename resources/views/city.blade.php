    ...
    $(document).ready(function() {
        ...
        $('#btnCreate').on('click', function(e) {
            ...
            if ($("#formCreate").valid()) {
                ...
                $.ajax({
                    type: "POST",
                    url: "http://191.252.192.67/cidades/adicionar",
                    data: dados,
                    dataType: 'json',
                    headers: {
        ...
        $('#btnUpdate').on('click', function(e) {
            ...
            if ($("#formEdit").valid()) {
                ...
                $.ajax({
                    type: "PUT",
                    url: "http://191.252.192.67/cidades/atualizar/" + dados[1]['value'],
                    data: dados,
                    dataType: 'json',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $('#btnDelete').on('click', function(e) {
            ...
            if ($("#formDelete").valid()) {
                ...
                $.ajax({
                    type: "DELETE",
                    url: "http://191.252.192.67/cidades/delete/" + dados[1]['value'],
                    data: dados,
                    dataType: 'json',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    ...
                    success: function(data) {
                        if (data.message === "Cidade excluida com sucesso") {
                            var modal = document.getElementById("delete-city");
                            modal.classList.add("hide", "fade");
                            Swal.fire({
                                icon: 'success',
